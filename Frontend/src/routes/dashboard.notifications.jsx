...
const Route = createFileRoute("/dashboard/notifications")({ component: Page });
...
function Page() {
  ...
  // Listen to incoming notifications via Socket in real-time!
  useSocket((newNotif) => {
  ...
  const markAllRead = async () => {
    try {
      ...
      // Dispatch storage / local event to update shell count
            ...
            return (
              ...
              >
                {isDeleting ? (
                  ...
                  <>
                    <Bell className={`h-5 w-5 ${n.read ? "text-muted-foreground" : "text-primary animate-pulse"}`} />
