...
const empty = { id: "", name: "", email: "", phone: "", address: "", tag: "Homeowner", joined: (/* @__PURE__ */ new Date()).toISOString().slice(0, 10) };
function Page() {
  const [items, setItems] = useStore("customers", []);
  ...
  return <div className="space-y-6">
      <PageHeader title="Customers" subtitle={`${items.length} total`} action={<button onClick={() => setEditing({ ...empty })} className="inline-flex items-center gap-2 rounded-full bg-primary px-5 py-2.5 text-sm font-medium text-primary-foreground hover:bg-primary/90"><Plus className="h-4 w-4" /> Add customer</button>} />
      ...
      <div className="overflow-hidden rounded-2xl border border-border bg-card">
        <table className="w-full text-sm">
          ...
          <tbody>
            {filtered.map((c) => <tr key={c.id} className="border-t border-border">
      ...
      {editing && <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4" onClick={() => setEditing(null)}>
    ...
    { k: "address", l: "Address" }
                  ...
                  <input value={editing[f.k]} onChange={(e) => setEditing({ ...editing, [f.k]: e.target.value })} className="mt-1 w-full rounded-lg border border-input bg-background px-3 py-2 text-sm outline-none focus:border-primary" />
                </div>)}
              <div>
                <label className="text-xs font-medium">Tag</label>
                <select value={editing.tag} onChange={(e) => setEditing({ ...editing, tag: e.target.value })} className="mt-1 w-full rounded-lg border border-input bg-background px-3 py-2 text-sm">
